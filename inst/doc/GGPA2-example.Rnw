...
pdftitle={GGPA2 Vignette}]
{hyperref}

\title{Multi-Disease Genetic Analysis Using `\texttt{GGPA2}' Package}
...
%\VignetteIndexEntry{GGPA2}
%\VignetteKeywords{GGPA2}
%\VignettePackage{GGPA2}
\maketitle
...
This vignette provides an introduction to the genetic analysis using the `\texttt{GGPA2}' package.
R package `\texttt{GGPA2}' implements graph-GPA,
a flexible statistical framework for the joint analysis of multiple genome-wide association studies (GWAS)
using a hidden Markov random field architecture. We encourage questions or requests regarding `\texttt{GGPA2}' package to be posted on our Google group for the GPA Suite \url{https://groups.google.com/d/forum/gpa-user-group}. Users can find the most up-to-date versions of `\texttt{GGPA2}' package in our GitHub webpage (\url{http://dongjunchung.github.io/GGPA2/}).
...
Section \ref{workflow} describes the overall graph-GPA analysis workflow \cite{GGPA2} including model fitting (Section \ref{fitting}), association mapping (Section \ref{association}), and visualization of an estimate phenotype graph (Section \ref{plot}).
...
We are now ready to fit a graph-GPA model using the GWAS $p$-value data described above (\texttt{simulation\$pmat}). R package \texttt{GGPA2} provides flexible analysis framework and automatically adjusts its model structure based on the provided data. Users can fit the graph-GPA model with the command:
...
fit <- GGPA2( pmat, pgraph )
@

\begin{thebibliography}{99}
\bibitem{GGPA2} Deng Q, Nam JH, Yilmaz AS, Chang W, Pietrzak M, Li L, Kim HJ, and Chung D (2022), ``graph-GPA 2.0: A graphical model for multi-disease analysis of GWAS results with integration of functional annotation data.''
